<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('posts.store', ['id' => $current_folder_id]) }}">
            @csrf
            <p>CreateTask</p>
            <textarea
                name="message"
            ...
            <x-primary-button class="mt-4 mb-4">{{ __('save') }}</x-primary-button>
        </form>
        <form method="POST" action="{{ route('folders.create') }}">
            ...
            @foreach($folders as $folder)
                <x-secondary-button class="mt-4">
                    <a href="{{ route('posts.index', ['id' => $folder->id]) }}" class="{{ $current_folder_id === $folder->id ? 'active' : '' }}">
                {{ $folder->title }}
                </x-secondary-button>
            @endforeach
